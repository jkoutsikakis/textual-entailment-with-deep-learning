...
class DAMModel(nn.Module):
    ...
    def __init__(self,
        ...
        projected_size = mlp_hidden_size
        ...
        self._att_mlp = pw.modules.MLP(
            ...
            hidden_layer_init=lambda w, _: torch.nn.init.xavier_uniform_(w),
            output_layer_init=lambda w, _: torch.nn.init.xavier_uniform_(w)
        )

        self._comp_mlp = pw.modules.MLP(
            input_size=2 * projected_size,
            num_hidden_layers=mlp_num_layers,
            hidden_layer_size=mlp_hidden_size,
            hidden_activation=mlp_activation,
            hidden_dp=mlp_dp,
    ...
    def forward(self, prems_indexes, prem_lens, hypos_indexes, hypo_lens):
        ...
        # print(horizontal_softmaxed[0])
        # argmaxed = torch.argmax(horizontal_softmaxed, dim=-1).tolist()[0]
        # prems_indexes = prems_indexes.tolist()[0]
        # hypos_indexes = hypos_indexes.tolist()[0]
        # for i in range(len(prems_indexes)):
        #     print(i2w[prems_indexes[i]], i2w[hypos_indexes[argmaxed[i]]])
